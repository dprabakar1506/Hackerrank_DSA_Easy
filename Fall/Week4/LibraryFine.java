...
class Result {
    ...
    // Case 1: Returned later than expected year - maximum fine
    if (y1 > y2) {
        fine = 10000;
    }
    // Case 2: Returned in the same year but later than expected month - monthly fine
    else if (y1 == y2 && m1 > m2) {
        fine = 500 * (m1 - m2);
    }
    // Case 3: Returned in the same month and year but later than expected day - daily fine
    else if (y1 == y2 && m1 == m2 && d1 > d2) {
        fine = 15 * (d1 - d2);
    }
    // Case 4: Returned on or before due date - no fine
    else {
        fine = 0;
    }
