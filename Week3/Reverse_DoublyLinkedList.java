...
class Result {
    ...
    // Traverse and swap next and prev
    while (current != null) {
        temp = current.prev;
        current.prev = current.next;
        current.next = temp;
        current = current.prev; // because next and prev are swapped
    }

    // After the loop, temp points to the old previous node (which is now the new head)
    if (temp != null) {
        llist = temp.prev;
    }
